...
contract InsightsPayment is Ownable, ReentrancyGuard {
    ...
    uint256 public constant LIFETIME_ACCESS_FEE = 6_900_000_000_000_000_000; // 6.9 cUSD
    ...
    mapping(address => bool) public lifetimeAccess;
    ...
    constructor(address _cUSD) Ownable(msg.sender) { // ✅ fixed
    ...
    function subscribe() external nonReentrant {
        require(!lifetimeAccess[msg.sender], "Lifetime access already active");
    ...
    /// @notice Purchase lifetime access with a one-time cUSD payment
    function buyLifetimeAccess() external nonReentrant {
        require(!lifetimeAccess[msg.sender], "Lifetime access already active");
        cUSD.safeTransferFrom(msg.sender, address(this), LIFETIME_ACCESS_FEE);
        lifetimeAccess[msg.sender] = true;
        emit LifetimeAccessGranted(msg.sender);
    }
    ...
    /// @notice Returns true if the user has lifetime access
    function hasLifetimeAccess(address user) external view returns (bool) {
        return lifetimeAccess[user];
    }
